...
"""Streamlit UI for EchoAnalytics – Phase 7 (Query Tab + Shared Memory)."""
import streamlit as st
import pandas as pd
...
# Page Config
# --------------------
st.set_page_config(page_title="EchoAnalytics – Phase 7", layout="wide")
st.title("📊 EchoAnalytics — Voice/Text Data Assistant")
...
if uploaded_file:
    ...
    # Store globally so Cleaning tab can access it too
    st.session_state["df"] = df

    st.subheader("📄 Dataset Preview")
    st.dataframe(df.head())
    
    st.info("You can now clean your dataset in the Cleaning tab.")
